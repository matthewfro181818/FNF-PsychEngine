...
class Character extends FlxSprite {
	...
	**/
	...
	#if FLX_ANIMATE
	public var swfAnim:FlxAnimate; // internal SWF renderer
	...
	public function loadCharacterFile(json:Dynamic) {
		...
		override function update(elapsed:Float) {
			...
			if (isSWF && swfAnim != null) {
				...
				swfAnim.x = this.x;
				swfAnim.y = this.y;
				swfAnim.flipX = this.flipX;

				// if Character.scale is changed externally
				var finalScale = this.scale.x * swfRenderScale;
				if (swfAnim.scale.x != finalScale || swfAnim.scale.y != finalScale) {
					swfAnim.scale.set(finalScale, finalScale);
					swfAnim.updateHitbox();
				}

				swfAnim.scrollFactor.set(scrollFactor.x, scrollFactor.y);
				swfAnim.antialiasing = this.antialiasing;
		...
		public function playAnim(AnimName:String, Force:Bool = false, Reversed:Bool = false, Frame:Int = 0):Void {
			...
			#if FLX_ANIMATE
			if (isSWF && swfAnim != null) {
				// Play SWF timeline by logical name
				swfAnim.anim.play(name, true);
				// Apply per-anim offset
				var off = _swfOffsets.exists(name) ? _swfOffsets.get(name) : FlxPoint.get();
				swfAnim.offset.set(off.x, off.y);
				return;
			}
			#end
			if (!isAnimateAtlas) {
		...
		public override function draw() {
			...
			#if FLX_ANIMATE
			if (isSWF && swfAnim != null) {
				// draw the SWF instead of the base sprite
				swfAnim.cameras = this.cameras; // respect camera lists
				swfAnim.draw();
				return;
			}
			#end
